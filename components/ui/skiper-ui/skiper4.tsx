...
export const ThemeToggleButton2 = ({
  ...
  className?: string;
  /** Controlled dark state. When provided, the button reflects this value. */
  isDark?: boolean;
  /** Called on click. When provided, overrides the internal toggle. */
  onToggle?: () => void;
}) => {
  const [internalDark, setInternalDark] = useState(false);
  const isDark = isDarkProp ?? internalDark;
  const handleClick = () => (onToggle ? onToggle() : setInternalDark((v) => !v));
  return (
    <button
      type="button"
      onClick={handleClick}
